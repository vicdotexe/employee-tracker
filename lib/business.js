class BusinessDatabase{
    ...
    async addDepartment(deptName){
        return await this.query(`INSERT INTO departments (name) VALUES (?)`,[deptName]);
    }

    async addRole(title, salary, departmentId){
        return await this.query(`INSERT INTO roles (title,salary,department_id) VALUES (?, ?, ?)`, [title,salary,departmentId]);
    }

    async addEmployee(firstName, lastName, roleId, managerId){
        return await this.query(`INSERT INTO employees (first_name, last_name, role_id, manager_id) VALUES (?, ?, ?, ?)`,[firstName,lastName,roleId,managerId||null]);
    }

    async updateEmployeeRole(employeeId, roleId){
        return await this.query('UPDATE employees SET role_id = ? WHERE id = ?', [roleId, employeeId]);
    }

    async updateEmployeeManager(employeeId, managerId){
        return await this.query('UPDATE employees SET manager_id = ? WHERE id = ?', [managerId, employeeId]);
    }
